...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COPYDATA:
	{
		...
		cout << "Zmin=" << length << endl;
		int i = 10;
		char out0[128] = { 0 };
		sprintf_s(out0, "%d\r", length);
		OutputDebugStringA(out0);//4fe7bc(nSelectsat数组地址)

		for (size_t i = 0; i < length/65536; i++)
		{
			int index = 65536 * i;
			memcpy(szBuffer, 
				((unsigned char*)pCopyData->lpData + index),
				size * sizeof(unsigned char));   //拷贝数组
		}
...
void OutputDebugPrintf(const char *strOutputString,int l)
{
